...
/** 新建一个链表node并返回 */
StudentLinkedListNode* studentLinkedListCreate(char student_id[], int grade) {   //OK
    StudentLinkedListNode *nod=(StudentLinkedListNode*) malloc (sizeof(StudentLinkedListNode));  //分配内存
    memcpy(nod->StuID,student_id,11);  //写入数据
    nod->Grade = grade;
    nod->next = NULL;
    return nod;
...
StudentLinkedListNode* studentLinkedListAdd(StudentLinkedListNode* head, StudentLinkedListNode* node) {    //OK
    if(node==NULL)//防止传入空节点
        return 0;
    node->next = head;
    return node;
...
/** 反转链表 */
StudentLinkedListNode* reverseLinkedList(StudentLinkedListNode*head){    //OK
    if(head == NULL) return 0;  //防止传入空节点
    if(head->next == NULL) return head;  //单个节点则返回该节点
    StudentLinkedListNode *p = head->next;
    StudentLinkedListNode *s = p->next;
    StudentLinkedListNode *q = head;
    while(s!=NULL)
    {
        p->next = q;
        q = p;
...
StudentLinkedListNode* findCrossBeginNode(StudentLinkedListNode* class1, StudentLinkedListNode* class2) {
    ...
    for(StudentLinkedListNode *p = class1;p!=NULL;p=p->next)
    {
        for(StudentLinkedListNode *q = class2;q!=NULL;q= q->next)
        {
            if(q == p)
            return q;
        }
